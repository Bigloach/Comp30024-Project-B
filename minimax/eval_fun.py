...
def evaluate_state(board: AgentBoard, color: PlayerColor) -> float:
    ...
    for frog in player_frogs:
        row_sum += frog[0]
        col_frogs[frog[1]] += 1
        distance = frog[0] if color == BLUE else (BOARD_N - 1 - frog[0])
    ...
    for frog in opponent_frogs:
        distance = frog[0] if color == RED else (BOARD_N - 1 - frog[0])
    ...
    target_r_penalty = abs(average_row - (BOARD_N - 1 if color == RED else 0))
