...
class MCTS_node:
    ...
    def update_backwards(self, winner: PlayerColor):
        """
        ...
        while curr_node:
            ...
            if winner is not None and curr_node.turn_color == winner:
                curr_node.wins += 1
...
def search_best_action(
    ...
    while iterations < MAX_ITER:
        ...
        if not curr_node.board.is_game_over() and curr_node is not None:
            if curr_node.actions is None or len(curr_node.actions) > 0:
                curr_node = curr_node.expand()

        winner = simulation(curr_node.board.copy(), curr_node.turn_color)
        curr_node.update_backwards(winner)
        iterations += 1
